...
class NeuralNetwork:
    ...
    def compute_cost(self, a_last, y):
        ...
        if self.l2_lambda != 0:
            ...
            l2_cost = (self.l2_lambda / (2 * batch_size)) * reduce(lambda ws, w: ws + cp.sum(cp.square(w)), weights, 0)
            return cost + l2_cost
        else:
            return cost
    ...
    def train(self, x_train, y_train, mini_batch_size, learning_rate, num_epochs, validation_data):
        ...
        for e in range(num_epochs):
            ...
            t3=ti.time()
            #print('YVAL ',y_val)
            accuracy = cp.sum(cp.argmax(self.predict(x_val), axis=1) == y_val) / x_val.shape[0]
            if accuracy>best_acc:
              best_acc=accuracy
              best_e=e
            #print('Predicted: ', cp.argmax(self.predict(x_val), axis=1))
            t4=ti.time()
