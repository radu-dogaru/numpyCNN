...
class RMSProp(Optimizer):
    ...
    def initialize(self):
        for layer in self.trainable_layers:
            ...
            w_shape = w.shape
            b_shape = b.shape
            self.s[('dw', layer)] = cp.zeros(w_shape)
            self.s[('db', layer)] = cp.zeros(b_shape)

    def update(self, learning_rate, w_grads, b_grads, step):
        s_corrected = {}
        s_correction_term = 1 - cp.power(self.beta, step)

        for layer in self.trainable_layers:
            layer_dw = ('dw', layer)
            layer_db = ('db', layer)

            self.s[layer_dw] = (self.beta * self.s[layer_dw] + (1 - self.beta) * cp.square(w_grads[layer]))
            self.s[layer_db] = (self.beta * self.s[layer_db] + (1 - self.beta) * cp.square(b_grads[layer]))

            s_corrected[layer_dw] = self.s[layer_dw] / s_correction_term
            s_corrected[layer_db] = self.s[layer_db] / s_correction_term

            dw = (learning_rate * (w_grads[layer] / (cp.sqrt(s_corrected[layer_dw]) + self.epsilon)))
            db = (learning_rate * (b_grads[layer] / (cp.sqrt(s_corrected[layer_db]) + self.epsilon)))
...
class Adam(Optimizer):
    ...
    def initialize(self):
        for layer in self.trainable_layers:
            ...
            w_shape = w.shape
            b_shape = b.shape
            self.v[('dw', layer)] = cp.zeros(w_shape)
            self.v[('db', layer)] = cp.zeros(b_shape)
            self.s[('dw', layer)] = cp.zeros(w_shape)
            self.s[('db', layer)] = cp.zeros(b_shape)

    def update(self, learning_rate, w_grads, b_grads, step):
        v_correction_term = 1 - cp.power(self.beta1, step)
        s_correction_term = 1 - cp.power(self.beta2, step)
        s_corrected = {}
        v_corrected = {}
        ...
        for layer in self.trainable_layers:
            ...
            self.s[layer_dw] = (self.beta2 * self.s[layer_dw] + (1 - self.beta2) * cp.square(w_grads[layer]))
            self.s[layer_db] = (self.beta2 * self.s[layer_db] + (1 - self.beta2) * cp.square(b_grads[layer]))

            s_corrected[layer_dw] = self.s[layer_dw] / s_correction_term
            s_corrected[layer_db] = self.s[layer_db] / s_correction_term

            dw = (learning_rate * v_corrected[layer_dw] / (cp.sqrt(s_corrected[layer_dw]) + self.epsilon))
            db = (learning_rate * v_corrected[layer_db] / (cp.sqrt(s_corrected[layer_db]) + self.epsilon))
