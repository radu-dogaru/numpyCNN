...
class Pool(Layer):
    ...
    def forward(self, a_prev, training):
        ...
        a = cp.zeros((batch_size, self.n_h, self.n_w, self.n_c))
        ...
        for i in range(self.n_h):
            ...
            for j in range(self.n_w):
                ...
                if self.mode == 'max':
                    ...
                    a[:, i, j, :] = cp.max(a_prev_slice, axis=(1, 2))

                elif self.mode == 'average':
                    a[:, i, j, :] = cp.mean(a_prev[:, v_start:v_end, h_start:h_end, :], axis=(1, 2))
        ...
        for i in range(self.n_h):
            ...
            for j in range(self.n_w):
                ...
                if self.mode == 'max':
                    ...
                    mean_value = cp.copy(da[:, i:i+1, j:j+1, :])
                    mean_value[:, :, :, cp.arange(mean_value.shape[-1])] /= (self.pool_size * self.pool_size)
    ...
    def cache_max_mask(self, x, ij):
        ...
        ax1, ax2 = cp.indices((x.shape[0], x.shape[3]))
